...
proc draw*(modelBatch: var ModelBatch, texture: Texture) =
  ...
  var
    ...
    model = mat4[GLfloat]()
    proj = perspective[GLfloat](45, getWidth() / getHeight(), 0.1, 1000.0)
  ...
  modelBatch.shader.setUniform1f("gDirectionalLight.Base.AmbientIntensity", 1.0)
...
proc draw*(modelBatch: var ModelBatch, m: model.Model, translation: Vec3f = vec3f(0,0,0), angle: float = 0, axis: Vec3f = vec3f(1, 0, 0), scale: Vec3f = vec3f(1.0)) =
...
proc draw*(modelBatch: var ModelBatch, m: var skinned_model.Model, translation: Vec3f = vec3f(0,0,0), angle: float = 0, axis: Vec3f = vec3f(1, 0, 0), scale: Vec3f = vec3f(1.0)) =
  ...
  boneTransform(m, float(getTicks()) * 0.001, transforms)

  for i in 0..<transforms.len:
    modelBatch.shader.setUniformMatrix("gBones[" & $i & "]", transforms[i], true);   
