...
proc createShader(shaderProgram: ShaderProgram, `type`: GLenum, source: string) : bool =
...
proc linkProgram(shaderProgram: var ShaderProgram) : bool =
  ...
  glDeleteShader(shaderProgram.vertexShader.handle)
  glDeleteShader(shaderProgram.fragmentShader.handle)
...
proc createShaderProgram*(vertexShaderSource: string, fragmentShaderSource: string) : ShaderProgram =
...
proc dispose*(shaderProgram: ShaderProgram) =
...
proc setUniform4f*(shaderProgram: ShaderProgram, name: string, value: Vec4f) =
  glUniform4f(GLint(glGetUniformLocation(shaderProgram.handle, name)),value.x, value.y, value.z, value.w)
